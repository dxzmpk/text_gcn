...
#         "data/dependency_clean_norm/ind.{}.test.index".format(dataset_str))
...
def load_corpus(dataset_str):
    ...
    objects = []
    read_dir = project.file_of_experiment
    print_log('reading data from'+read_dir)
    for i in range(len(names)):
        with open((read_dir+"ind.{}.{}").format(dataset_str, names[i]), 'rb') as f:
            if sys.version_info > (3, 0):
                objects.append(pkl.load(f, encoding='latin1'))
            else:
                objects.append(pkl.load(f))
    ...
    train_idx_orig = parse_index_file(
        read_dir+"{}.train.index".format(dataset_str))
...
def preprocess_adj(adj):
    ...
    adj_normalized = normalize_adj(adj + sp.eye(adj.shape[0])) # 防止被0除现象
...
def print_log(msg='', end='\n'):
    ...
    file = os.path.join(project.utils_dir,project.experiment_name)

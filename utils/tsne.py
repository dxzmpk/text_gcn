...
data_set = 'ohsumed' # 20ng R8 R52 ohsumed mr
data_path = project.file_of_experiment
...
f = open(os.path.join(project.train_dir,project.experiment_name+'.doc_vectors.txt'), 'r')
lines = f.readlines()
f.close()
...
# plt.title(md_file)
plt.tight_layout()
plt.savefig(os.path.join(project.analysis_dir, project.experiment_name + '.tsne.png'))
plt.show()
